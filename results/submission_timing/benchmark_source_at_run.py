...
def benchmark_field(a, prediction, source, truth, rng, repeats, field_seed, budgets):
    ...
    def adaptive_correction(rhs):
        adaptive_result[0] = adaptive_poisson_cg(A, rhs, float(a.min()))
        return adaptive_result[0].correction

    corrections["adaptive_rank90"] = adaptive_correction
    for k in budgets:
    ...
    for repeat in range(repeats):
        for name in rng.permutation(list(corrections)):
            ...
            if name == "adaptive_rank90":
                result = adaptive_result[0]
...
def summarize(frame, bootstrap, seed):
    ...
    for (n, kappa), group in frame.groupby(["N", "kappa"]):
        ...
        out[f"N{n}_k{kappa:g}"] = {"N": int(n), "kappa": float(kappa),
                                   "n_fields": len(by_field), "methods": methods}
    return out
...
def run(args):
    ...
    source_paths = [Path(__file__).resolve(), REPO / "solver/adaptive.py",
                    ...
                    REPO / "uncertainty/rank_bounds.py"]
    ...
    payload = {
        "protocol": {"scope": "assembled A and neural prediction already available; residual included for every method",
                     ...
                     "threads": 1, "runtime_warmup": "one discarded field per N/kappa; fresh-A setup remains timed"},
