...
Family B is a support-breaking checkerboard family for later OOD diagnostics.
...
def family_C_field(
    ...
    *,
    ...
    original Family-C distribution. Tilting them (more/larger inclusions, higher
    ``channel_prob``) at FIXED ``kappa``/``a_min`` produces a mild *in-family*
    covariate shift: the binary-contrast features (log_a_max/contrast) are
    unchanged, only the volume-fraction/interface features shift, so the shift
    keeps OVERLAPPING support with the default distribution (used for the
    on-support weighted-CP regime).
    """
...
def family_C_sample(
    ...
    **field_kwargs,
    ...
    are forwarded to :func:`family_C_field` for the in-family on-support shift.
    """
...
def family_B_field(
    ...
    """Binary checkerboard/support-breaking coefficient field."""
